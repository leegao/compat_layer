...
static SpvReflectDescriptorType
ClassifyHeapDescriptorType(const SpvReflectTypeDescription *p_type) {
  if (IsNull(p_type)) {
  ...
  case SpvOpTypeImage: {
    ...
    if (p_type->traits.image.dim == SpvDimBuffer) {
      return is_storage ? SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER
                        : SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
    }
    return is_storage ? SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_IMAGE
...
FindAccessChainBaseVariable(SpvReflectPrvParser *p_parser,
  ...
  while ((base_node->op != SpvOpVariable) &&
         (base_node->op != SpvOpUntypedVariableKHR)) {
    switch (base_node->op) {
    case SpvOpLoad: {
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser *p_parser) {
  ...
  while (spirv_word_index < p_parser->spirv_word_count) {
    ...
    if (op == SpvOpAccessChain || op == SpvOpInBoundsAccessChain ||
        op == SpvOpUntypedAccessChainKHR ||
        op == SpvOpUntypedInBoundsAccessChainKHR ||
        op == SpvOpUntypedPtrAccessChainKHR ||
        op == SpvOpUntypedInBoundsPtrAccessChainKHR) {
  ...
  while (spirv_word_index < p_parser->spirv_word_count) {
    ...
    case SpvOpUntypedInBoundsPtrAccessChainKHR: {
      // Untyped access chain layout: Result Type, Result Id, Base Type, Base,
      // [Element,] Indexes. PtrAccessChain variants have an extra Element
      // operand at word 5; indexes follow.
      CHECKED_READU32(p_parser, p_node->word_offset + 1,
                      p_node->result_type_id);
      CHECKED_READU32(p_parser, p_node->word_offset + 2, p_node->result_id);
      SpvReflectPrvAccessChain *p_access_chain =
      ...
      p_access_chain->index_count = (node_word_count > index_first_word)
                                        ? (node_word_count - index_first_word)
                                        : 0;
      if (p_access_chain->index_count > 0) {
        ...
        for (uint32_t index_index = 0;
          ...
          CHECKED_READU32(p_parser,
                          p_node->word_offset + index_first_word + index_index,
                          index_id);
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser *p_parser) {
  ...
  for (uint32_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if ((p_node->op != SpvOpDecorate) && (p_node->op != SpvOpMemberDecorate) &&
        ...
        (p_node->op != SpvOpMemberDecorateIdEXT) &&
    ...
    case SpvDecorationArrayStrideIdEXT: {
      // ArrayStride but with constant ID instead of literal (mainly used for
      // spec constants)
      uint32_t word_offset = p_node->word_offset + member_offset + 3;
      uint32_t constant_id = 0;
      CHECKED_READU32(p_parser, word_offset, constant_id);
      SpvReflectPrvNode *p_constant_node = FindNode(p_parser, constant_id);
      if (IsNotNull(p_constant_node) &&
          (p_constant_node->op == SpvOpConstant)) {
        CHECKED_READU32(p_parser, p_constant_node->word_offset + 3,
                        p_target_decorations->array_stride);
      }
    } break;
    ...
    case SpvDecorationOffsetIdEXT: {
      // Offset but with constant ID instead of literal (mainly used for spec
      // constants)
      uint32_t word_offset = p_node->word_offset + member_offset + 3;
      uint32_t constant_id = 0;
      CHECKED_READU32(p_parser, word_offset, constant_id);
...
ParseDescriptorBindings(SpvReflectPrvParser *p_parser,
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if ((p_node->decorations.set.value == (uint32_t)INVALID_VALUE) ||
        (p_node->decorations.binding.value == (uint32_t)INVALID_VALUE)) {
      continue;
    }
...
static SpvReflectResult
ParseEntryPointHeapAccesses(SpvReflectPrvParser *p_parser,
                            SpvReflectShaderModule *p_module) {
  ...
  SpvReflectPrvNode **pp_heap_vars =
      (SpvReflectPrvNode **)calloc(heap_var_count, sizeof(*pp_heap_vars));
  ...
  for (uint32_t ep_idx = 0; ep_idx < p_module->entry_point_count; ++ep_idx) {
    ...
    SpvReflectPrvHeapAccess *p_scratch = (SpvReflectPrvHeapAccess *)calloc(
        untyped_access_chain_count, sizeof(*p_scratch));
    if (IsNull(p_scratch)) {
      SafeFree(p_called_functions);
      SafeFree(pp_heap_vars);
      return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      ...
      if (!SearchSortedUint32(p_called_functions, called_function_count,
                              current_function_id)) {
        continue;
      }
      ...
      uint32_t base_id = 0;
      if (ReadU32(p_parser, p_node->word_offset + 3, &data_type_id) !=
          SPV_REFLECT_RESULT_SUCCESS) {
        continue;
      }
      if (ReadU32(p_parser, p_node->word_offset + 4, &base_id) !=
          SPV_REFLECT_RESULT_SUCCESS) {
        continue;
      }
      ...
      for (uint32_t s = 0; s < scratch_count; ++s) {
        if (p_scratch[s].heap_var_idx == heap_idx &&
            p_scratch[s].runtime_array_type_id == data_type_id) {
          duplicate = true;
          break;
        }
      }
      ...
      ++scratch_count;
      if (pp_heap_vars[heap_idx]->decorations.built_in ==
    ...
    if (resource_count > 0) {
      p_entry->resource_heap_accesses =
          (SpvReflectEntryPointResourceHeapAccess *)calloc(
              resource_count, sizeof(*p_entry->resource_heap_accesses));
    ...
    if (sampler_count > 0) {
      p_entry->sampler_heap_accesses =
          (SpvReflectEntryPointSamplerHeapAccess *)calloc(
              sampler_count, sizeof(*p_entry->sampler_heap_accesses));
    ...
    for (uint32_t s = 0; s < scratch_count; ++s) {
      ...
      SpvReflectPrvNode *p_arr =
          FindNode(p_parser, p_scratch[s].runtime_array_type_id);
      ...
      if (IsNotNull(p_arr)) {
        ...
        if (IsNotNull(p_type_desc) && p_type_desc->op == SpvOpTypeBufferEXT &&
          ...
          for (size_t n = 0; n < p_parser->node_count; ++n) {
            ...
            if (ReadU32(p_parser, p_bp->word_offset + 3, &source_id) !=
                SPV_REFLECT_RESULT_SUCCESS) {
              continue;
            }
            ...
            SpvReflectTypeDescription *p_struct =
                FindType(p_module, p_ptr->type_id);
      ...
      if (pp_heap_vars[v]->decorations.built_in == SpvBuiltInResourceHeapEXT) {
        SpvReflectEntryPointResourceHeapAccess *p_acc =
            &p_entry->resource_heap_accesses[r_idx++];
        ...
        SpvReflectEntryPointSamplerHeapAccess *p_acc =
            &p_entry->sampler_heap_accesses[s_idx++];
        p_acc->heap_name = pp_heap_vars[v]->name;
        p_acc->runtime_array_type_id = p_scratch[s].runtime_array_type_id;
        p_acc->stride = p_scratch[s].stride;
        p_acc->type_description = p_type_desc;
